...
/*   Updated: 2017/11/19 20:35:05 by vrybalko         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
t_sector	*parse(char *path)
{
	...
	s[0].floor = 0;
	s[0].num_vertices = 7;
	s[0].vertices = (t_vertex*)malloc(sizeof(t_vertex) * 7);
	s[0].neighbors = (int*)malloc(sizeof(int) * 7);
	for (int i = 0; i < 7; i++)
		s[0].neighbors[i] = -1;
	s[0].neighbors[4] = 1;
	...
	s[0].vertices[4] = vertex(10, -5);
	s[0].vertices[5] = vertex(10, 5);
	s[0].vertices[6] = vertex(10, 10);
	s[1].ceil = 9.5;
	s[1].floor = 0.5;
